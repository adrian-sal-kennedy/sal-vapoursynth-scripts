...
def squarepix(clip, height=576, crop=1.0, aspect=16 / 9, outaspect=16 / 9, sd_mode=702, off_w=0, off_h=0, scale=1):
    ...
    opad: int = (-spadx * width / sw).__int__()
...
def SDtoHD(clip, crop=1.0, aspect=16 / 9, sd_mode=702, hq=True, off_w=0, off_h=0, scale=1, hd720=False):
    ...
    if hq:
        sd_mode = sd_mode * 2
        clip = clip.std.Transpose().nnedi3.nnedi3(0, dh=True).std.Transpose().nnedi3.nnedi3(0, dh=True).fmtc.resample(
            sx=.5, sy=.5, css="444")
    outaspect = 16 / 9
    inaspect = aspect
    par = outaspect / inaspect
...
def conditionalDeint(f, orig, deint):
    if f.props._Combed:
        return deint
    else:
        return orig
...
def fieldmatchproc(f, clip, swapped):
    if f[0].props.sameAverage > f[1].props.swapAverage:
        ...
        swapped = core.std.FrameEval(swapped, functools.partial(conditionalDeint, orig=swapped, deint=swappeddeint),
                                     swappedprop)
        return swapped
...
def fieldmatch(clip):
    ...
    clip = core.std.FrameEval(clip, functools.partial(fieldmatchproc, clip=clip, swapped=swapped),
                              prop_src=[samestat, swapstat])
...
def deinterlace(clip, dbl=False, order=1, usefield="noodle"):
    ...
    v1 = core.tdm.TDeintMod(clip, order=order, field=usefield, mode=yadmode, length=10, mtype=2, ttype=5, nt=2,
                            edeint=edeint)
...
def dvnr(clip, sad=300, scd1=75, scd2=50, blk=8, pel=2):
    ...
    degrained = core.std.AverageFrames(interleaved, [1, 1, 1])
...
def dvnr3(clip, sad=390, scd1=200, scd2=50, blk=8, pel=4):
    ...
    blur = core.bilateral.Bilateral(blur, ref=blur, sigmaS=16 * v1.height / 1080, sigmaR=.025, algorithm=2,
                                    planes=[1, 2])
    vsuper = core.mv.Super(v1, pel=pel)
...
def dvnr5(clip, sad=390, scd1=200, scd2=50, blk=16, pel=4):
    ...
    # degrained = core.dgm.DegrainMedian(interleaved, 16, mode=1)
    degrained = core.dgm.DegrainMedian(interleaved, 255, mode=4).std.AverageFrames([1, 1, 1, 1, 1])
...
def dvnr7(clip, sad=250, scd1=200, scd2=50, blk=16, pel=4):
    ...
    degrained = core.dgm.DegrainMedian(interleaved, 255, mode=4).std.AverageFrames([1, 1, 1, 1, 1, 1, 1])
    v1 = core.std.SelectEvery(degrained, 7, 3)
    return v1


def RemoveDirt(inclip, repmode=16):
    cleansed = core.rgvs.Clense(inclip)
    sbegin = core.rgvs.ForwardClense(inclip)
    send = core.rgvs.BackwardClense(inclip)
    scenechange = core.rgvs.SCSelect(inclip, sbegin, send, cleansed)
    alt = core.rgvs.Repair(scenechange, inclip, mode=[repmode, repmode, 1])
    restore = core.rgvs.Repair(cleansed, inclip, mode=[repmode, repmode, 1])
    corrected = core.rgvs.RestoreMotionBlocks(cleansed, restore, neighbour=inclip, alternative=alt, gmthreshold=70,
                                              dist=1, dmode=2, noise=10, noisy=12, grey=0)
...
def dvnrmed(clip, sad=150, scd1=200, scd2=33, blk=8, pel=2, show=False, multiplier=8, threshold=0.05):
    ...
    lowpass = core.resize.Bicubic(v1, width=32, height=32, filter_param_a=1, filter_param_b=0).focus.TemporalSoften(1,
                                                                                                                    255,
                                                                                                                    255,
                                                                                                                    254,
                                                                                                                    2).resize.Bicubic(
        width=v1.width, height=v1.height)
    ...
    degrained = core.std.AverageFrames(interleaved, [1, 1, 1])
    mask = core.std.Expr(clips=[wiped, degrained],
                         expr=["x y - abs " + str(threshold) + " 65536 * - " + str(multiplier) + " *", "32768",
                               "32768"]).std.Maximum().resize.Bicubic(int(v1.width / 2), int(v1.height / 2),
                                                                      prefer_props=False).std.Maximum().resize.Bicubic(
        v1.width, v1.height, filter_param_a=1, filter_param_b=0, prefer_props=False)
...
def fpsblur(clip, fpsnum=24000, fpsden=1000, sad=300, scd1=1000, scd2=50, blk=16, pel=2, samples=9, shutter=0.5,
            exponent=10, stabilize=True, ml=200):
    scd2 = int(255 * (scd2 / 100))
    ...
    if stabilize:
        ...
        v1 = core.mv.DepanStabilise(v1, data=pandata, dxmax=32, dymax=32, zoommax=1.2, rotmax=2, mirror=15, blur=0,
                                    method=1)
    ...
    compf = core.mv.Compensate(v1, vsuper, vf, thsad=sad, thscd1=scd1, thscd2=scd2)
    compb = core.mv.Compensate(v1, vsuper, vb, thsad=sad, thscd1=scd1, thscd2=scd2)
    denoise = core.std.AverageFrames([compf, v1, compb], [1, 1, 1])
    ...
    hifps = core.std.AverageFrames(hifps, np.full(samples, 1))
...
def qfpsblur(clip, fpsnum=24000, fpsden=1000, samples=9, shutter=0.5, exponent=10, stabilize=True, fast=True,
             denoise=False):
    ssamples = int(samples / shutter)
    exp = str(exponent)

    v1 = clip
    lofi = core.resize.Lanczos(clip, format=vs.YUV444P8, matrix_s="709", transfer_s="709", primaries_s="709",
                               matrix_in_s="170m", transfer_in_s="601", primaries_in_s="170m")
    ...
    if denoise:
        ...
        v1 = core.std.AverageFrames([compf, v1, compb], [1, 1, 1])
    msk1 = core.mv.BlockFPS(clip=v1, super=vsuper, mvbw=vb, mvfw=vf, num=fpsnum * ssamples, den=fpsden, blend=False,
                            mode=5, ml=10)
    fps1 = core.mv.FlowFPS(clip=v1, super=vsuperlinlight, mvbw=vb, mvfw=vf, num=fpsnum * ssamples, den=fpsden,
                           blend=False)
    ...
    msk3 = core.mv.BlockFPS(clip=v1, super=vsuper, mvbw=vb3, mvfw=vf3, num=fpsnum * ssamples, den=fpsden, blend=False,
                            mode=5, ml=10)
    fps3 = core.mv.FlowFPS(clip=v1, super=vsuperlinlight, mvbw=vb, mvfw=vf, num=fpsnum * ssamples, den=fpsden,
                           blend=False)
...
def ledfilter(clip, period=59.4, fwidth=9, forder=3, startrow=10, mthreshold=0.002, soft=5):
    ...
    filtered = core.std.Transpose(filtered).vcfreq.F1Quiver(filter=[4, f1, f3, forder], morph=0, custom=0, test=0,
                                                            strow=startrow, nrows=24, gamma=0.15).std.Transpose()
    filtered = core.std.ShufflePlanes(clips=[filtered], planes=[0, 1, 2], colorfamily=vs.YUV)
    protectmask = core.std.Expr([v1, filtered], expr=["x y - abs " + str(mthreshold) + " > 0 1 ?", "", ""]).std.BoxBlur(
        planes=[1, 0], hradius=rad, hpasses=passes, vradius=rad, vpasses=passes).std.ShufflePlanes(planes=[0, 0, 0],
                                                                                                   colorfamily=vs.YUV)
    v1 = core.std.Expr([v1, filtered, protectmask],
                       expr=["x 1 z - * y z * +", "x 1 z - * y z * +", "x 1 z - * y z * +"])
